...
class DirBrute:
    ...
    def create_widgets(self, tab):
        ...
        self.url_entry = ctk.CTkEntry(setting_frame, placeholder_text="http://example.com", height=30)
        ...
        self.start_btn = ctk.CTkButton(btn_frame, text="▶ Start", command=self.start_brute,
                                      fg_color="#2ecc71", hover_color="#27ae60", height=35, width=100,
                                      font=ctk.CTkFont(weight="bold"))
        self.start_btn.pack(side=ctk.LEFT, padx=5)
        
        self.stop_btn = ctk.CTkButton(btn_frame, text="⏹ Stop", command=self.stop_brute,
    ...
    def save_log(self):
        ...
        filename = f"dir_brute_log_{int(time.time())}.txt"
    ...
    def start_brute(self):
        if not self.url_entry.get() or not self.wordlist_entry.get():
            messagebox.showerror("Error", "Please enter URL and select wordlist")
            return
        ...
        self.progress_bar.set(0)
        self.clear_log()
        self.log_to_widget("[*] Starting Directory Brute Force")
        self.app.set_status("Directory brute force started...")
    ...
    def _brute_thread(self):
        try:
                    ...
                    if result:
                        ...
                        if status == 200:
                            self.app.vulnerabilities.append({
                                'title': f'Directory Found: {word}',
                                'description': f'URL: {url}/{word}\nStatus: {status}\nSize: {size} bytes',
                                'severity': 'Low',
            ...
            self.app.set_status("Directory brute finished")
