...
class SQLi:
    ...
    def create_widgets(self, tab):
        ...
        self.url_entry = ctk.CTkEntry(setting_frame, placeholder_text="http://example.com/page.php?id=1", height=30)
        self.url_entry.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        ctk.CTkLabel(setting_frame, text="📝 Parameter:", font=ctk.CTkFont(size=12)).grid(row=1, column=0, padx=5, pady=5)
        self.param_entry = ctk.CTkEntry(setting_frame, placeholder_text="id", height=30)
    ...
    def save_log(self):
        ...
        filename = f"sqli_log_{int(time.time())}.txt"
    ...
    def start_scan(self):
        ...
        self.progress_bar.set(0)
        self.clear_log()
        self.log_to_widget("[*] Starting SQL Injection Scanner")
        self.app.set_status("SQL Injection scan started...")
    ...
    def _scan_thread(self):
        try:
                ...
                try:
                    ...
                    if len(r.text) > 0:
                        if "mysql" in r.text.lower() or "sql" in r.text.lower() or "error" in r.text.lower():
                            found = True
                            self.log_to_widget(f"[!] SQL Injection found: {payload}")
                            self.log_to_widget(f"[!] Response: {r.text[:200]}")
                            ...
                            self.log_to_widget(f"[*] Time-based: {payload} ({elapsed:.2f}s)")
                except:
                    pass
            ...
            self.log_to_widget("-" * 50)
            if found:
                self.log_to_widget("[!] SQL Injection detected!")
            else:
                self.log_to_widget("[*] No SQL injection found")
            ...
            self.app.set_status("SQL Injection scan finished")
