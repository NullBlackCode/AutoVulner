...
class SSHCracker:
    ...
    def create_widgets(self, tab):
        ...
        self.progress_bar.grid(row=4, column=0, padx=5, pady=5, columnspan=4)
        ...
        self.honeypot_label = ctk.CTkLabel(stat_row, text="🪤 Honeypots: 0", text_color="#ff6b6b", font=ctk.CTkFont(size=11))
        self.honeypot_label.pack(side=ctk.LEFT, padx=10)
    ...
    def save_log(self):
        ...
        filename = f"ssh_log_{int(time.time())}.txt"
    ...
    def start_attack(self):
        ...
        self.honeypot_label.configure(text="🪤 Honeypots: 0")
        self.progress_bar.set(0)
        self.clear_log()
        self.log_to_widget("[*] Starting SSH Cracking")
        self.app.set_status("SSH Cracking started...")
    ...
    def _attack_thread(self):
        try:
            ...
            if self.user_type.get() == "Single":
                users = [self.user_entry.get().strip() or "root"]
            else:
                    ...
                    for j, pwd in enumerate(passwords):
                        ...
                        try:
                            ...
                            if "Connection refused" in str(e) or "timed out" in str(e):
                                ...
                                honeypots += 1
                                self.honeypot_label.configure(text=f"🪤 Honeypots: {honeypots}")
                                self.log_to_widget(f"[!] Honeypot: {ip}")
                        
                        progress = (i * len(users) * len(passwords) + users.index(user) * len(passwords) + j) / total
                        self.progress_bar.set(progress)
            
            self.log_to_widget("-" * 50)
            self.log_to_widget(f"[*] Done. Success: {found}, Failed: {failed}, Honeypots: {honeypots}")
            ...
            self.app.set_status("SSH Cracking finished")
